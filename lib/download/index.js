...
import { mergeAV, remuxToMp4, ffmpegAvailable } from '../ffmpeg.js';
...
export async function downloadResources(resources, opts) {
  const { outDir, concurrency = 6, keepParts = false, userAgent, verbose = false, log = () => {} } = opts;
  ...
  if (pair) {
    ...
    await downloadDirect(pair.video, vPath, { userAgent });
    await downloadDirect(pair.audio, aPath, { userAgent });
    if (ffmpegAvailable()) {
...
async function downloadOne(res, opts) {
  const { outDir, format = 'mp4', concurrency = 6, nameTpl = '{title}.{ext}', keepParts = false, userAgent, verbose = false, log = () => {} } = opts;
  ...
  await downloadDirect(res, outPath, { userAgent });
  return outPath;
}
