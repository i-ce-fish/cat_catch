...
// asarUnpack 会把真实二进制文件复制到 app.asar.unpacked，
// 但 ffmpeg-static 用 __dirname 拼出的路径字符串仍停留在虚拟的 app.asar 内，
// spawn() 走真实文件系统识别不了 asar，需要手动纠正，否则打包后会报 spawn ENOTDIR。
const resolvedFfmpegPath =
  typeof ffmpegPath === 'string' ? ffmpegPath.replace('app.asar', 'app.asar.unpacked') : ffmpegPath;

export function ffmpegAvailable() {
  return typeof resolvedFfmpegPath === 'string' && resolvedFfmpegPath.length > 0;
}

function runFfmpeg(args, verbose = false) {
  return new Promise((resolve, reject) => {
    const proc = spawn(resolvedFfmpegPath, ['-hide_banner', '-loglevel', verbose ? 'info' : 'error', ...args], {
